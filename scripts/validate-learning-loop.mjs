...
try {
  ...
  await page.getByRole("cell", { name: /AAPL/ }).waitFor();
  const search = page.getByRole("combobox", { name: "Instrument" });
  await search.fill("bond");
  await page.getByRole("option", { name: /CZGB35/ }).waitFor();
  await search.press("ArrowDown");
  await search.press("Enter");
  await page.getByLabel("Quantity").fill("1.3333");
  await button(page, "Buy").click();
  await page.getByRole("status").filter({ hasText: "Purchase complete" }).waitFor();
  await page.getByRole("cell", { name: /CZGB35/ }).waitFor();
  const bondRow = page.getByRole("row").filter({ hasText: "CZGB35" });
  await bondRow.getByRole("button", { name: "Sell", exact: true }).click();
  await page.getByLabel("Quantity to sell").fill("0.3333");
  ...
  await page.getByRole("row").filter({ hasText: "CZGB35" }).getByText("1", { exact: true }).waitFor();
