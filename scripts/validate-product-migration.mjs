...
try {
  ...
  const instrumentSearch = page.getByRole("combobox", { name: "Instrument" });
  ...
  await button("Buy").click(); await page.getByRole("status").filter({ hasText: "Purchase complete" }).waitFor();
  await page.getByRole("row").filter({ hasText: "AAPL" }).waitFor();
  assert.equal(await page.getByTestId("portfolio-cash").textContent(), "5,351.62 Kč");
  await page.getByRole("row").filter({ hasText: "AAPL" }).getByRole("button", { name: "Sell", exact: true }).click();
  await page.getByLabel("Quantity to sell").fill("0.1");
  await page.locator('section[aria-labelledby="sell-heading"]').getByRole("button", { name: "Sell", exact: true }).click(); await page.getByRole("status").filter({ hasText: "Sale complete" }).waitFor();
  ...
  await button("Reset portfolio").click();
  ...
  assert.equal(await page.getByTestId("portfolio-cash").textContent(), "6,000 Kč");
  await page.reload(); await heading("Portfolio Lab");
  assert.equal(await page.getByRole("row").filter({ hasText: "AAPL" }).count(), 0, "reset persists");
