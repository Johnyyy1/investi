...
try {
  ...
  await page.goto(`${baseURL}/sign-up`); await heading("Create account");
  ...
  await page.waitForURL("**/learn"); await heading("Continue learning");
  [{ id: userId }] = await sql`select id from "user" where email = ${email}`;
  await layouts("home-empty");
  await heading("Continue learning"); await layouts("learn");
  ...
  assert.equal(await page.getByLabel("Získaný virtuální kapitál: 2,000 Kč").count(), 1, "first completion presents the server-awarded Practice Capital");
  ...
  assert.equal(await repeatAttempt.getByLabel("Získaný virtuální kapitál: 2,000 Kč").count(), 0, "repeated completion does not present another reward");
  ...
  await page.getByRole("link", { name: "View module", exact: true }).click(); await heading("Returns & Compounding");
  assert.match(await page.getByTestId("module-progress").textContent(), /1 of 3/);
  ...
  await button("Mark lesson complete").click(); await heading("Lesson complete"); await page.getByRole("link", { name: "View module", exact: true }).click(); await heading("Returns & Compounding");
  ...
  const instrumentSearch = investmentDialog.getByRole("combobox", { name: "Search investments" });
  await instrumentSearch.fill("AAPL");
  await investmentDialog.getByRole("option", { name: /AAPL/ }).waitFor();
  await instrumentSearch.press("Enter");
  await investmentDialog.getByRole("textbox", { name: "Quantity", exact: true }).fill("0.25");
  await investmentDialog.getByRole("button", { name: "Review order", exact: true }).click();
  await investmentDialog.getByRole("button", { name: "Confirm buy AAPL", exact: true }).click(); await page.getByRole("status").filter({ hasText: "Investment added" }).waitFor();
  await page.getByTestId("holding-AAPL").waitFor();
  assert.equal(await page.getByTestId("portfolio-cash").textContent(), "5,351.62 Kč");
  await page.getByTestId("holding-AAPL").getByRole("button", { name: "Actions for AAPL", exact: true }).click();
  await page.getByRole("menu", { name: "Actions for AAPL" }).getByRole("menuitem", { name: "Sell", exact: true }).click();
  const saleDialog = page.getByRole("dialog", { name: "Sell AAPL" });
  await saleDialog.getByLabel("Quantity to sell").fill("0.1");
  await saleDialog.getByRole("button", { name: "Sell AAPL", exact: true }).click(); await page.getByRole("status").filter({ hasText: "Investment sold" }).waitFor();
  await page.reload(); await heading("Portfolio Lab");
  await page.getByTestId("holding-AAPL").getByText("0.15 shares", { exact: true }).waitFor();
  await layouts("portfolio-populated");
  await page.evaluate(() => { document.documentElement.style.fontSize = "200%"; });
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false, "Portfolio respects 200% text");
  await page.evaluate(() => { document.documentElement.style.fontSize = ""; });
  await button("Portfolio options").click();
  await page.getByRole("menu", { name: "Portfolio options" }).getByRole("menuitem", { name: "Reset portfolio", exact: true }).click();
  await page.getByRole("dialog").getByRole("button", { name: "Reset portfolio", exact: true }).click();
  ...
  await page.getByTestId("portfolio-cash").filter({ hasText: "6,000 Kč" }).waitFor();
  await page.reload(); await heading("Portfolio Lab");
  assert.equal(await page.getByTestId("holding-AAPL").count(), 0, "reset persists");
  await page.getByRole("navigation", { name: "Main navigation" }).getByRole("link", { name: "Learn", exact: true }).click(); await heading("Continue learning"); await layouts("home-completed");
  await page.getByRole("link", { name: "Progress", exact: true }).click(); await heading("Look how far you’ve come.");
  ...
  await page.getByLabel("Account menu").click(); await button("Sign out").click(); await heading("Welcome back"); await layouts("sign-in");
  await signIn("incorrect-password"); await page.getByRole("alert").filter({ hasText: /.+/ }).waitFor();
  await signIn(); await page.waitForURL("**/learn"); await heading("Continue learning");
  ...
  console.log("PASS: sign-up, auth errors, lessons 1–3, persistent portfolio buy/sell/reload/reset, Home, Progress, sign-out/in, review, unavailable routes, all six widths. Screenshots:", screenshotDir);
} finally {
  await browser.close();
