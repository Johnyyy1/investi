...
export default async function LabPage() {
  ...
  return <PageFrame width="standard">
    ...
    <div className="mt-3"><AppHeader title="What happens if…" description="Change something. See the effect. Understand why." /></div>
    <section className="mt-12 grid gap-8 border-y border-ql-border py-10 sm:grid-cols-[1fr_1fr] sm:items-center">
      <div><h2 className="text-ql-section font-semibold">Build a portfolio</h2><p className="mt-3 max-w-sm text-ql-body text-ql-secondary">Use the Practice Capital you earn to buy sample investments and understand what you own.</p>{unlock && !unlock.unlocked ? <p className="mt-4 inline-flex items-center gap-2 text-ql-small font-semibold text-ql-secondary"><LockKeyhole aria-hidden="true" className="size-4" />Locked · Complete Investing Foundations · {unlock.totalXp} / {unlock.xpRequired} XP</p> : null}<LearningLink href="/lab/portfolio" className="mt-6 w-full sm:w-auto">{unlock?.unlocked ? "Open Portfolio Lab" : "View unlock requirements"}<ArrowRight aria-hidden="true" className="size-4" /></LearningLink></div>
