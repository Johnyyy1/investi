...
export function AchievementBadge({ title, description, unlocked }: { title: string; description: string; unlocked: boolean }) {
  ...
  return <motion.div initial={false} animate={{ opacity: unlocked ? 1 : 0.8 }} transition={{ duration }} className="flex items-start gap-3">
    <span className={`flex size-12 shrink-0 items-center justify-center rounded-ql-md ${unlocked ? "bg-ql-warning-bg text-ql-warning-ink" : "bg-ql-subtle text-ql-secondary"}`}><Icon className="size-5" aria-hidden="true" /></span>
