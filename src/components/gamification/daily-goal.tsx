...
export function DailyGoal({ completed, target }: { completed: number; target: number }) {
  ...
  const lessonLabel = target === 1 ? "lekce" : target >= 2 && target <= 4 ? "lekce" : "lekcí";
  return <section aria-label="Dnešní studijní cíl" className="rounded-surface border border-border bg-surface/70 px-5 py-4 sm:px-6">
    <div className="flex flex-wrap items-center justify-between gap-2 text-small">
      ...
      <span className={complete ? "min-w-0 text-right tabular-nums font-bold text-success-ink" : "min-w-0 text-right tabular-nums text-secondary"}>{completed} / {target} {lessonLabel}</span>
    </div>
    ...
    <p className="mt-2 text-microcopy text-secondary">{complete ? "Cíl splněn — skvělé tempo." : "Dokonči lekci a posuň dnešní cíl."}</p>
  </section>;
}
