...
export function PortfolioLab({ portfolio }: { portfolio: PortfolioView }) {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => startSearch(async () => {
      ...
      if (!response.ok) setSheetError(response.message);
    }), 180);
    return () => window.clearTimeout(timeout);
  }, [query]);
  ...
  return <>
    <header className="mt-3 flex flex-wrap items-start justify-between gap-5">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="rounded-pill border border-info/35 bg-info-soft px-3 py-1 text-microcopy font-bold text-info-ink">Sample data</span>
        </div>
    ...
    <div className="mt-8 space-y-10 lg:mt-10">
      ...
      {!hasCapital ? <ZeroCapitalState portfolio={portfolio} /> : <>
        ...
        {!portfolio.valuationComplete ? <Feedback state="warning" role="status">Some sample quotes or FX rates are unavailable, so portfolio valuation is incomplete. Missing values are not counted as zero.</Feedback> : null}
    ...
    <dialog ref={sellDialog} aria-labelledby="sell-heading" aria-describedby="sell-description" onClose={() => { setSellHolding(null); setSheetError(null); restoreTriggerFocus(sellReturnFocus.current, primaryInvestRef.current); requestAnimationFrame(() => restoreTriggerFocus(sellReturnFocus.current, primaryInvestRef.current)); }} onCancel={(event) => { if (mutationPending) event.preventDefault(); }} className="fixed inset-y-0 right-0 left-auto m-0 h-[100dvh] max-h-none w-full max-w-none overflow-y-auto border-0 border-l border-border bg-surface p-0 text-foreground shadow-elevation-2 backdrop:bg-foreground/35 sm:max-w-[31rem] sm:rounded-l-panel">
      {sellHolding ? <div className="flex min-h-full flex-col">
        ...
        <div className="flex-1 px-[min(1.25rem,5vw)] py-6 sm:px-7 sm:py-8">
          ...
          <p className="mt-5 text-small text-secondary">Uses the displayed educational sample price. The server validates the final trade.</p>
        </div>
...
function SearchStep({ query, results, activeIndex, pending, searchRef, onQueryChange, onActiveIndexChange, onChoose }: { query: string; results: readonly InstrumentSearchResult[]; activeIndex: number; pending: boolean; searchRef: RefObject<HTMLInputElement | null>; onQueryChange: (value: string) => void; onActiveIndexChange: (index: number) => void; onChoose: (instrumentId: string) => void }) {
  return <section aria-labelledby="search-investments-title"><h3 id="search-investments-title" className="text-section-title font-bold">Find an investment</h3><p className="mt-2 text-body text-secondary">Search the educational dataset by company, fund, bond, or symbol.</p><div className="relative mt-6"><label htmlFor="instrument-search" className="text-small font-bold">Search investments</label><div className="relative mt-2"><Search aria-hidden="true" className="pointer-events-none absolute top-3.5 left-4 size-5 text-secondary" /><Input ref={searchRef} id="instrument-search" role="combobox" aria-autocomplete="list" aria-expanded={results.length > 0} aria-controls="instrument-results" aria-activedescendant={activeIndex >= 0 ? `instrument-option-${activeIndex}` : undefined} aria-busy={pending || undefined} autoComplete="off" value={query} placeholder="Search stocks, ETFs, bonds…" className="pr-12 pl-12" onChange={(event) => onQueryChange(event.target.value)} onKeyDown={(event) => { if (!results.length) return; if (event.key === "ArrowDown") { event.preventDefault(); onActiveIndexChange((activeIndex + 1) % results.length); } if (event.key === "ArrowUp") { event.preventDefault(); onActiveIndexChange((activeIndex - 1 + results.length) % results.length); } if (event.key === "Enter" && activeIndex >= 0) { event.preventDefault(); onChoose(results[activeIndex].instrumentId); } if (event.key === "Escape") { event.stopPropagation(); onActiveIndexChange(-1); } }} />{pending ? <span className="absolute top-3.5 right-4 text-small text-secondary">…</span> : null}</div></div>{results.length > 0 ? <ul id="instrument-results" role="listbox" aria-label="Instrument search results" className="mt-3 divide-y divide-border overflow-hidden rounded-surface border border-border bg-surface shadow-elevation-1">{results.map((result, index) => <li key={result.instrumentId} id={`instrument-option-${index}`} role="option" aria-selected={index === activeIndex} aria-label={`${result.symbol} ${result.name}, ${assetTypeLabel(result.assetType)}, ${result.quoteCurrency}, Sample data`} className="aria-selected:bg-primary-soft"><button type="button" tabIndex={-1} className="flex min-h-16 w-full flex-col items-start gap-2 px-4 py-3 text-left hover:bg-primary-soft sm:flex-row sm:items-center sm:justify-between" onMouseDown={(event) => event.preventDefault()} onClick={() => onChoose(result.instrumentId)}><span className="w-full min-w-0 sm:w-auto"><strong className="block">{result.symbol}</strong><span className="block truncate text-small text-secondary">{result.name}</span></span><span className="w-full shrink-0 text-left text-microcopy text-secondary sm:w-auto sm:text-right"><span className="block font-semibold">{assetTypeLabel(result.assetType)} · {result.quoteCurrency}</span><span className="block">Sample data</span></span></button></li>)}</ul> : query.trim() && !pending ? <p role="status" className="mt-5 text-small text-secondary">No sample investments found. Try a company name or symbol.</p> : <div className="mt-8 border-t border-border pt-6"><p className="text-small font-bold">Try a search</p><p className="mt-1 text-small text-secondary">AAPL, Microsoft, world ETF, or bond</p></div>}</section>;
}
