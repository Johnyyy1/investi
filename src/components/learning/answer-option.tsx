...
export function AnswerOption({ name, value, state = "idle", checked, disabled = false, onChange, children, shortcut }: {
  name: string; value: string; state?: AnswerState; checked?: boolean; disabled?: boolean;
  ...
  return <label className={cn("relative flex min-h-14 cursor-pointer items-center gap-3 rounded-ql-md border-2 px-4 py-3 text-ql-body transition-colors duration-[var(--ql-motion-fast)] has-[:focus-visible]:outline-3 has-[:focus-visible]:outline-offset-3 has-[:focus-visible]:outline-ql-link", states[state], unavailable && "cursor-default")}>
    <input className="size-4 shrink-0 accent-ql-link" type="radio" name={name} value={value} checked={checked ?? state === "selected"} disabled={unavailable} onChange={() => onChange?.(value)} aria-describedby={state === "correct" || state === "incorrect" ? id : undefined} />
