...
export function ContinueLearning({ summary }: { summary: ReturnType<typeof getLearnerSummary> }) {
  ...
  return <section aria-labelledby="continue-heading" className="rounded-ql-xl border border-ql-border bg-ql-surface p-6 shadow-ql-sm sm:p-8">
    <p className="text-ql-small font-semibold text-ql-link">Returns &amp; Compounding · {next.estimatedMinutes} min</p>
    ...
    <p className="mt-2 text-ql-emphasis">{allComplete ? "You’ve completed every available lesson in Returns." : next.title}</p>
    ...
    <LearningLink href={`/learn/returns/${next.slug}`} className="mt-6">{allComplete ? "Review compounding" : state?.status === "in_progress" ? "Continue lesson" : "Start lesson"}<ArrowRight aria-hidden="true" className="size-5" /></LearningLink>
  </section>;
}
