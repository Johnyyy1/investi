...
export function MetricResult({ label, value, sentiment = "neutral", note }: {
  ...
  return <div><p className="text-ql-small text-ql-secondary">{label}</p><p className={`mt-1 text-ql-section font-semibold tabular-nums ${sentimentColor[sentiment]}`}>{value}</p>{note ? <p className="mt-1 text-ql-meta text-ql-secondary">{note}</p> : null}</div>;
}
