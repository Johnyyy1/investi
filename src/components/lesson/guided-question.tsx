...
export function GuidedQuestion({ block, onContinue, pending }: { block: QuestionBlock; onContinue: () => void; pending: boolean }) {
  ...
  return <section aria-labelledby={`${block.id}-question`} className="mt-8">
    ...
    {feedback === undefined ? <div className="sticky bottom-0 z-20 border-t border-ql-border bg-ql-surface px-6 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:px-10"><LearningButton className="w-full sm:w-auto" disabled={evaluation === undefined || pending} onClick={() => setFeedback(evaluation)}>Check answer</LearningButton></div> : <LessonFeedback sticky autoFocusAction state={feedback ? "correct" : "incorrect"} title={feedback ? "That’s right" : "Look at the changing base"} onContinue={onContinue} actionPending={pending}>{feedback ? block.correctExplanation : block.incorrectExplanation}</LessonFeedback>}
  </section>;
}
