...
function renderBlock(block: LessonBlock) {
  switch (block.type) {
    ...
    case "interactiveFigure": return <div key={block.id}><p className="mt-8 text-base leading-7 text-neutral-700">{block.description}</p>{block.figure === "return-calculator" ? <ReturnCalculator /> : block.figure === "price-series-explorer" ? <PriceSeriesExplorer /> : block.figure === "compounding-explorer" ? <CompoundingExplorer /> : <RecoveryExplorer />}</div>;
    case "multipleChoiceQuestion": case "numericQuestion": case "multiNumericQuestion": return <Question key={block.id} block={block} />;
