...
import { DeterministicEtfAnalyticsProvider } from "./etf-analytics";
...
export function createDeterministicMarketDataService(options: MarketDataServiceOptions = {}) {
  ...
  return new MarketDataService(new DeterministicMarketDataProvider(clock), { ...options, clock, equityFundamentalsProvider: options.equityFundamentalsProvider ?? new DeterministicEquityFundamentalsProvider(clock), etfAnalyticsProvider: options.etfAnalyticsProvider ?? new DeterministicEtfAnalyticsProvider(clock) });
}
