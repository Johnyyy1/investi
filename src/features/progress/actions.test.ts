...
describe("shared persisted lesson actions", () => {
  ...
  it.each([["foundations-stocks", "module-investing-foundations"], ["returns-compounding", "module-returns"]])("counts the correct module for %s", async (id, moduleId) => {
  ...
  it.each(["foundations-bonds-cash", "returns-log-returns", "unknown"])("rejects writes to %s", async (id) => {
